...
const PROVEEDORES_QUERY = gql`
  query Proveedores($page: Int, $limit: Int, $search: String) {
    proveedores(page: $page, limit: $limit, search: $search) {
      items {
        id
        ruc
        nombre
...
const CREAR_PROVEEDOR = gql`
  mutation CrearProveedor($input: CrearProveedorInput!) {
...
const ACTUALIZAR_PROVEEDOR = gql`
  mutation ActualizarProveedor($id: ID!, $input: ActualizarProveedorInput!) {
...
const TOGGLE_PROVEEDOR = gql`
  mutation ToggleProveedor($id: ID!) {
    toggleProveedor(id: $id) {
      id
      activo
    }
  }
...
export function ProveedoresPage() {
  ...
  const { data, loading, refetch } = useQuery(PROVEEDORES_QUERY, {
    variables: { page, limit: LIMIT, search: search || undefined },
    fetchPolicy: "cache-and-network",
  });

  const [crearProveedor, { loading: creando }] = useMutation(CREAR_PROVEEDOR);
  const [actualizarProveedor, { loading: actualizando }] = useMutation(ACTUALIZAR_PROVEEDOR);
  const [toggleProveedor] = useMutation(TOGGLE_PROVEEDOR);

  const proveedores: Proveedor[] = data?.proveedores?.items ?? [];
  const total: number = data?.proveedores?.total ?? 0;
  ...
  function abrirEditar(p: Proveedor) {
    setEditandoId(p.id);
    setForm({ ruc: p.ruc, nombre: p.nombre, contacto: p.contacto ?? "" });
    setFormError("");
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editandoId) {
        await actualizarProveedor({ variables: { id: editandoId, input } });
      } else {
        await crearProveedor({ variables: { input } });
      }
      cerrarModal();
      refetch();
    } catch (err: unknown) {
  ...
  async function handleToggle(id: string) {
    await toggleProveedor({ variables: { id } });
    refetch();
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <button style={s.btnPrimary} onClick={abrirCrear}>+ Nuevo Proveedor</button>
      </div>
          ...
          {proveedores.length === 0 && !loading ? (
            <tr>
              <td colSpan={5} style={s.emptyRow}>
                {search ? `No se encontraron resultados para "${search}"` : "No hay proveedores registrados."}
              </td>
            </tr>
          ) : (
            proveedores.map((p) => (
              <tr key={p.id}>
                <td style={s.td}><strong>{p.ruc}</strong></td>
                <td style={s.td}>{p.nombre}</td>
                <td style={s.td}>{p.contacto || <span style={{ color: "#7a95aa" }}>—</span>}</td>
                <td style={s.td}>
      ...
      {totalPages > 1 && (
        <div style={s.pagination}>
          <span>{total} proveedor{total !== 1 ? "es" : ""}</span>
      ...
      {!totalPages || totalPages <= 1 ? (
        <div style={{ ...s.pagination, justifyContent: "flex-start" }}>
          <span style={{ color: "#7a95aa" }}>{total} proveedor{total !== 1 ? "es" : ""}</span>
        </div>
      ) : null}
      ...
      {modalOpen && (
        <div style={s.overlay} onClick={(e) => e.target === e.currentTarget && cerrarModal()}>
          <div style={s.modal}>
            <h2 style={s.modalTitle}>{editandoId ? "Editar Proveedor" : "Nuevo Proveedor"}</h2>
